...
/* USER CODE BEGIN 4 */
//void Process_Sensors(tsl_user_status_t status){
//	if ((MyTKeys[0].p_Data->StateId == TSL_STATEID_DETECT) || (MyTKeys[1].p_Data->StateId == TSL_STATEID_DETECT) || (MyTKeys[2].p_Data->StateId == TSL_STATEID_DETECT)) HAL_GPIO_WritePin(LD6_GPIO_Port, LD6_Pin, GPIO_PIN_SET);
//	else HAL_GPIO_WritePin(LD6_GPIO_Port, LD6_Pin, GPIO_PIN_RESET);
//
//	switch(status){
//
	...
	uint8_t cont = 0;
        ...
        for(cont = 0; cont <= 256; cont++)
        {
        	if(cont == 256)
        	{
        		for(int i = 0 ; i <= 255 ; i++)
        		{
        			SampleBuffer[i] = 0;  // Clean the Buffer
        		}
        	}
        	else
        	{
        		SampleBuffer[cont] =  MyTKeys[0].p_ChD->Meas;  //Fill the Buffer

        	}
        }


//     printf("%u", MyTKeys[0].p_ChD->Ref );
//     printf(",");
//
//     printf("%u", MyTKeys[0].p_ChD->Meas);
//     printf(",");
//
//     printf("DELTA: %u \r\n\n", MyTKeys[0].p_ChD->Delta);
//	   printf(",");
//
//	   printf("REFERENCE REST: %u \r\n", MyTKeys[0].p_ChD->RefRest );
//	   printf("FLAG: %u \r\n",  MyTKeys[0].p_ChD->Flags);
//	   printf("STATE ID: %u \r\n", MyTKeys[0].p_Data->StateId);
//	   printf("DETECTION TIME OUT COUNTER: %u \r\n", MyTKeys[0].p_Data->CounterDTO);
//	   printf("COUNTER DEBOUNCE: %u \r\n", MyTKeys[0].p_Data->CounterDebounce);
//
//	    switch(status){
//	    case TSL_USER_STATUS_OK_ECS_OFF:
